...
function patientByGameId(patients, gameID, gameType) {
    ...
    for (var i = 0; i < patients.length && found == false; i++) {
        for (var j = 0; j < patients[i].games.length; j++) {
            if (gameID == patients[i].games[j].id && gameType == patients[i].games[j].type) {
                result = patients[i];
                found = true;
                break;
            }
...
app.get('/getGameToDo', function (req, res) {
    ...
    async.map(filesPath, function (filePath, cb) { //reading files or dir
        ...
        console.log('type: ' + gameToDoType + '\nid: ' + gameToDoId);

        gameToDo = fitGameToPatient(gameToDo, gameToDoType, patientByGameId(patients, gameToDoId, gameToDoType));
...
app.get('/getGameById', function (req, res) {
    ...
    if ((gameType != null && gameType != undefined && gameType != "") && (gameID != null && gameID != undefined && gameID != "")) {
        ...
        async.map(filesPath, function (filePath, cb) { //reading files or dir
            ...
            var patient = patientByGameId(patients, gameID, gameType);
